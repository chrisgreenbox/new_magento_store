...
//                exit;
    ...
    {
        if (Mage::helper('mageworx_customoptions')->isCustomerGroupsEnabled()) {
            return $this;
        }
        $customerGroupId = Mage::helper('mageworx_customoptions')->getCustomerGroupId();
    ...
    {
        if (Mage::helper('mageworx_customoptions')->isStoreViewsEnabled()) {
            return $this;
        }
    ...
    protected function filterViewMode($collection, $item, $key)
    {
        ...
        if ($item->getViewMode() == self::OPTION_VIEWMODE_DISABLE && !is_null($item->getViewMode())) {
            $collection->removeItemByKey($key);
            return false;
        } elseif (!Mage::app()->getStore()->isAdmin()
                && ($item->getViewMode() == self::OPTION_VIEWMODE_BACKEND
                || $item->getViewMode() == self::OPTION_VIEWMODE_ADMINONLY)
            ) {
            $collection->removeItemByKey($key);
            return false;
        }
     ...
     * @return $this
     */
    protected function applyFilter($collection, $product)
    {
        foreach ($collection as $key => $item) {
            if (!$this->filterViewMode($collection, $item, $key)
    ...
    public function getProductOptionCollection(Mage_Catalog_Model_Product $product) {
        ...
        if ($action === NULL || $action->getFullActionName() == "catalog_category_view") {
        ...
        if (!$isProductEditPage) {
            // filter
            $this->applyFilter($collection, $product);
